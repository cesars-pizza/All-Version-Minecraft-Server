...
function GenerateBlocks(world, socket, chunkX, chunkZ) {
    ...
    var groundID = utils.registry.block.GetBlockID(world, socket.thisPlayer.selectedRegistries.block, "netherrack")
    var genericPlotFloorID = utils.registry.block.GetBlockID(world, socket.thisPlayer.selectedRegistries.block, "grass_block")
    var cornerID = utils.registry.block.GetBlockID(world, socket.thisPlayer.selectedRegistries.block, "iron_block")
    var northStairID = utils.registry.block.GetBlockID(world, socket.thisPlayer.selectedRegistries.block, "cobblestone_stairs[facing=north]")
    var eastStairID = utils.registry.block.GetBlockID(world, socket.thisPlayer.selectedRegistries.block, "cobblestone_stairs[facing=east]")
    var southStairID = utils.registry.block.GetBlockID(world, socket.thisPlayer.selectedRegistries.block, "cobblestone_stairs[facing=south]")
    var westStairID = utils.registry.block.GetBlockID(world, socket.thisPlayer.selectedRegistries.block, "cobblestone_stairs[facing=west]")
    ...
    if (chunkTypeX == 0 || chunkTypeZ == 0) {
        blocks[1][0][0] = cornerID
        blocks[1][15][0] = cornerID
        blocks[1][0][15] = cornerID
        blocks[1][15][15] = cornerID
...
function GetBlock(world, socket, blockPos) {
    if (blockPos.y == 0) return "netherrack"
        ...
        if (utils.math.NegMod(blockPos.x, 32) > 15 && utils.math.NegMod(blockPos.z, 32) > 15) {
            ...
            if ((utils.math.NegMod(blockPos.x, 32) > 0 && utils.math.NegMod(blockPos.x, 32) < 15) || (utils.math.NegMod(blockPos.z, 32) > 0 && utils.math.NegMod(blockPos.z, 32) < 15)) return "air"
                ...
                if (utils.math.NegMod(blockPos.x, 32) == 0 && utils.math.NegMod(blockPos.z, 32) >= 16) return "cobblestone_stairs[facing=west]"
                else if (utils.math.NegMod(blockPos.x, 32) == 15 && utils.math.NegMod(blockPos.z, 32) >= 16) return "cobblestone_stairs[facing=east]"
                else if (utils.math.NegMod(blockPos.z, 32) == 0 && utils.math.NegMod(blockPos.x, 32) >= 16) return "cobblestone_stairs[facing=south]"
                else if (utils.math.NegMod(blockPos.z, 32) == 15 && utils.math.NegMod(blockPos.x, 32) >= 16) return "cobblestone_stairs[facing=north]"
                else return "iron_block"
            }
        }
