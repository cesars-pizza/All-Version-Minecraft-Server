...
function PlayerMessage(socket, username, message) {
    packetWriter.Beta.Chat_Message(socket)(undefined, socket, `<${username}> ${message}`)
}
...
function JoinMessage(socket, username) {
    packetWriter.Beta.Chat_Message(socket)(undefined, socket, `§e${username} joined the game.`)
}
...
function QuitMessage(socket, username) {
    packetWriter.Beta.Chat_Message(socket)(undefined, socket, `§e${username} left the game.`)
}
...
function SystemMessage(socket, message) {
    packetWriter.Beta.Chat_Message(socket)(undefined, socket, `§d[System] ${message}`)
}
...
function ErrorMessage(socket, message) {
    packetWriter.Beta.Chat_Message(socket)(undefined, socket, `§c${message}`)
}
