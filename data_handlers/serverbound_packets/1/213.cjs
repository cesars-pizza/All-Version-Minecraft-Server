            ...
            if (!hasOpenInstance) {
                ...
                if (!socket.thisPlayer.verified) {
                    ...
                    if (socket.thisPlayer.joinCount % 2 == 0) {
                        ...
                        if (!world.config.suppressNonUniversalBlocks || world.universalRegistries.block.includes("oak_fence")) { packetWriter.Alpha.Add_To_Inventory(socket)(world, socket, utils.registry.item.GetItemID(world, socket.thisPlayer.selectedRegistries.item, "oak_fence"), 64); secondInvHasValidBlock = true }
