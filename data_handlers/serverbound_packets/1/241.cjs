        ...
        if (socket.disconnect == "") {
            ...
            if (!hasOpenInstance) {
                ...
                if (!socket.thisPlayer.verified) {
                    ...
                    packetWriter.Beta.Login_Response(socket)(world, socket, socket.thisPlayer.alphaID, world.config.serverName, world.config.serverStatus, 0, 0)
                    ...
                    for (var i = 0; i < world.loadedPlayers.length; i++) {
                        ...
                        packetWriter.Beta.Named_Entity_Spawn(socket)(socket, world.loadedPlayers[i].alphaID, world.loadedPlayers[i].username, world.loadedPlayers[i].position, world.loadedPlayers[i].rotation, utils.registry.item.GetItemID(world, socket.thisPlayer.selectedRegistries.item, world.loadedPlayers[i].inventory.held_item))
                    }
