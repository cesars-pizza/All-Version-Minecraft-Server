        ...
        if (socket.disconnect == "") {
                    ...
                    if (hitBuildIndex != undefined) {
                        ...
                        if (world.builds[hitBuildIndex].creator == socket.thisPlayer.username || world.builds[hitBuildIndex].settings.publicInteractions) {
                            ...
                            if (prevBlock.startsWith('oak_door') || prevBlock.startsWith('lever') || prevBlock.startsWith('stone_button')) {
                                ...
                                if (typeof(blockID) == "number") packetWriter.Beta.Block_Change(socket)(world, socket, blockPos, blockID, 0, false)
                                else packetWriter.Beta.Block_Change(socket)(world, socket, blockPos, blockID.id, blockID.metadata, false)
                            }
                        }
                    }
                }
            ...
            if (utils.math.NegMod(blockPos.x, 32) >= 16 && utils.math.NegMod(blockPos.z, 32) >= 16 && socket.thisPlayer.digging.ticks >= 6) {
                ...
                if (hitBuildIndex == undefined || world.builds[hitBuildIndex].creator == socket.thisPlayer.username) {
                    if (hitBuildIndex == undefined) {
                        ...
                        world.builds.push(utils.builds.GenerateBuild(socket)(world, Math.floor(blockPos.x / 32), Math.floor(blockPos.z / 32), socket.thisPlayer.username, socket.thisPlayer.uvni, socket.thisPlayer.settings.defaultBuildSettings))
                    }
            ...
            if (!updateSuccessful && blockPos.y < 64 && status.value == 3) {
                ...
                if (oldBlockUpdate == -1) {
                    ...
                    if (typeof(replacementBlock) == "number") packetWriter.Beta.Block_Change(socket)(world, socket, blockPos, replacementBlock, 0, false)
                    else packetWriter.Beta.Block_Change(socket)(world, socket, blockPos, replacementBlock.id, replacementBlock.metadata, false)
                }
            }
        }
