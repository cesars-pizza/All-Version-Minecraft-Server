...
function ReadPacket(world, socket, data) {
    ...
    if (splitIndex >= 0) {
        var posX = dataReader.readInt(socket, data, 1)
        ...
        var item = dataReader.readShort(socket, data, face.nextPos)

        if (item.value > 0) splitIndex -= 2

        if (splitIndex >= 0) {
            var itemCount = {value: 0}
            var itemMeta = {value: 0}
            if (item.value > 0) {
                var itemCount = dataReader.readByte(socket, data, item.nextPos)
                var itemMeta = dataReader.readByte(socket, data, itemCount.nextPos)
            }

            if (posX.value == -1 && posY.value == -1 && posZ.value == -1 && face.value == -1) return splitIndex

            if (socket.disconnect == "") {
                var placedName = utils.registry.item.GetItemName(world, socket.thisPlayer.selectedRegistries.item, `${item.value}:${itemMeta.value}`)
                ...
                if (utils.math.NegMod(facingBlock.x, 32) >= 16 && utils.math.NegMod(facingBlock.z, 32) >= 16) {
                    ...
                    if (hitBuildIndex != undefined) {
                        if (!socket.thisPlayer.sneaking) {
                            if (originalBlock.block.startsWith("chest")) {
                                packetWriter.Beta.Open_Window(socket)(socket, 3, 0, "Chest", 27)
                                return
                            } else if (originalBlock.block.startsWith("furnace")) {
                                packetWriter.Beta.Open_Window(socket)(socket, 2, 2, "Furnace", 3)
                                return
                            } else if (originalBlock.block.startsWith("crafting_table")) {
                                packetWriter.Beta.Open_Window(socket)(socket, 1, 1, "Crafting", 5)
                                return
                            }
                            ...
                            if (world.builds[hitBuildIndex].creator == socket.thisPlayer.username || world.builds[hitBuildIndex].settings.publicInteractions) {
                                if (originalBlock.block.startsWith("oak_door")) {
                                    var open = originalBlock.block.includes('open=true')
                                    var half = originalBlock.block.includes('half=upper') ? "upper" : "lower"
                                    var hinge = originalBlock.block.includes('hinge=right') ? "right" : "left"
                                    var facing = "north"
                                    if (originalBlock.block.includes('facing=east')) facing = "east"
                                    else if (originalBlock.block.includes('facing=south')) facing = "south"
                                    else if (originalBlock.block.includes('facing=west')) facing = "west"

                                    var newBlock = `oak_door[facing=${facing},hinge=${hinge},half=${half},open=${!open}]`

                                    utils.tick_actions.set_block.AddBlockUpdate(socket)(world, socket, originalBlock, newBlock, false, originalBlock.block)
                                    return
                                } else if (originalBlock.block.startsWith("lever")) {
                                    var powered = originalBlock.block.includes('powered=true')
                                    var face = "wall"
                                    if (originalBlock.block.includes('facing=ceiling')) facing = "ceiling"
                                    else if (originalBlock.block.includes('facing=floor')) facing = "floor"
                                    var facing = "north"
                                    if (originalBlock.block.includes('facing=east')) facing = "east"
                                    else if (originalBlock.block.includes('facing=south')) facing = "south"
                                    else if (originalBlock.block.includes('facing=west')) facing = "west"
                                    ...
                                    utils.tick_actions.set_block.AddBlockUpdate(socket)(world, socket, originalBlock, newBlock, false, originalBlock.block)
                                    return
                                } else if (originalBlock.block.startsWith("stone_button")) {
                                    var powered = originalBlock.block.includes('powered=true')
                                    var face = "wall"
                                    if (originalBlock.block.includes('facing=ceiling')) facing = "ceiling"
                                    else if (originalBlock.block.includes('facing=floor')) facing = "floor"
                                ...
                                if (originalBlock.block.startsWith('oak_door') || originalBlock.block.startsWith('lever') || originalBlock.block.startsWith('stone_button')) {
                                    var blockID = utils.registry.block.GetBlockID(world, socket.thisPlayer.selectedRegistries.block, originalBlock.block)
                                    if (typeof(blockID) == "number") packetWriter.Beta.Block_Change(socket)(world, socket, originalBlock, blockID, 0, false)
                                    else packetWriter.Beta.Block_Change(socket)(world, socket, originalBlock, blockID.id, blockID.metadata, false)
                                }
                            }
                        }
                    }
                    ...
                    if (hitBuildIndex == undefined || world.builds[hitBuildIndex].creator == socket.thisPlayer.username) {
                        if (hitBuildIndex == undefined) {
                            ...
                            world.builds.push(utils.builds.GenerateBuild(socket)(world, Math.floor(facingBlock.x / 32), Math.floor(facingBlock.z / 32), socket.thisPlayer.username, socket.thisPlayer.uvni, socket.thisPlayer.settings.defaultBuildSettings))
                        }
                        ...
                        if (facingBlock.y <= 1) {
                                    ...
                                    if (face.value == 1) {
                                        if (originalBlock.block == placedName) {
                                            if (utils.collisions.PlayerCollidingWithBlock(socket)(world, socket, socket.thisPlayer.position, originalBlock) != "inside") {
                                                ...
                                                if (facingBlockReplacable) packetWriter.Beta.Block_Change(socket)(world, socket, facingBlock, 0, 0, false)
                            ...
                            } else if (placedName == "nether_portal") {
                                var playerDirection = utils.angle.GetDirectionNESW(socket.thisPlayer.rotation.yaw)

                                var portalAxis = (playerDirection == "north" || playerDirection == "south") ? "x" : "z"

                                if (facingBlockReplacable) {
                                    if (validBlock) {
                                        utils.tick_actions.set_block.AddBlockUpdate(socket)(world, socket, facingBlock, placedName + `[axis=${portalAxis}]`, false, facingBlock.block)
                                        updateSuccessful = true
                                    } else socket.thisPlayer.tick.errorMessages.push("This block isn't available in all versions.")
                                }
                ...
                if (!updateSuccessful) {
                    ...
                    if (oldBlockUpdate == -1) {
                        ...
                        if (facingBlock.y < 64) {
                            ...
                            if (typeof(replaceBlock) == "number") packetWriter.Beta.Block_Change(socket)(world, socket, facingBlock, replaceBlock, 0, false)
                            else packetWriter.Beta.Block_Change(socket)(world, socket, facingBlock, replaceBlock.id, replaceBlock.metadata, false)
                        } else packetWriter.Beta.Block_Change(socket)(world, socket, facingBlock, 0, 0, false)
                    }
                } else {
                    if (facingBlock.y > 1) {
                        for (var i = 0; i < world.loadedPlayers.length; i++) {
