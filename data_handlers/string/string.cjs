...
function Read(socket, data, position) {
    ...
    if (socket.thisPlayer.upvn >= 8 && socket.thisPlayer.upvn <= 26) return require('./213.cjs').Read(data, position)
    else {
...
function Write(socket, value) {
    ...
    if (socket.thisPlayer.upvn >= 8 && socket.thisPlayer.upvn <= 26) return require('./213.cjs').Write(value)
    else {
