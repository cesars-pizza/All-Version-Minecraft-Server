...
function Read(socket, data, position) {
    if (socket.thisPlayer.upvn == -1) return require('./29.cjs').Read(data, position)
        ...
        socket.log(`ERR: Cannot Parse UByte for Version ${socket.thisPlayer.upvn}:${socket.thisPlayer.uvni}`)
        return {
            value: 0,
            length: 1,
            nextPos: position + 1
...
function Write(socket, value) {
    if (socket.thisPlayer.upvn == -1) return require('./29.cjs').Write(value)
        ...
        socket.log(`ERR: Cannot Write UByte for Version ${socket.thisPlayer.upvn}:${socket.thisPlayer.uvni}`)
        return [0]
    }
}
